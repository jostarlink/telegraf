class Composer {
  ...
  static hears (match, middleware) {
    return Composer.mount('text', Composer.match(normalizeMath(match), middleware))
  }

  static action (match, middleware) {
    return Composer.mount('callback_query', Composer.match(normalizeMath(match), middleware))
  }
  ...
  static compose (middlewares) {
    ...
    return (rootCtx, next) => {
      ...
      function execute (i, ctx) {
        ...
        try {
          return Promise.resolve(handler(ctx, (newCtx) => execute(i + 1, newCtx)))
        } catch (err) {
